...
Requires:
    - LibreOffice (soffice) on PATH or at tools/libreoffice/SourceDir/LibreOffice/program/soffice.exe
...
def find_soffice() -> str | None:
    ...
    bundled = repo_root / "tools" / "libreoffice" / "SourceDir" / "LibreOffice" / "program" / "soffice.exe"
    if bundled.exists():
        return str(bundled)
    return None
...
def extract_images(prs: Presentation, out_dir: Path) -> list[dict]:
        ...
        for shp in slide.shapes:
            if shp.shape_type == 13:  # PICTURE
                ...
                try:
                    ...
                    # Some embedded images are not standard ImagePart; skip them
                    print(f"  Warning: cannot read image on slide {s_idx}: {e}", file=sys.stderr)
                    continue
                out_path = out_dir / f"slide_{s_idx:03d}_img_{img_counter:02d}.{ext}"
...
def extract_tables(prs: Presentation, out_dir: Path) -> list[dict]:
    ...
    for s_idx, slide in enumerate(prs.slides, 1):
        ...
        for shp in slide.shapes:
            if shp.has_table:
                t_idx += 1
                data = table_to_data(shp.table)
                base_name = f"slide_{s_idx:03d}_table_{t_idx:02d}"
                ...
                json_path.write_text(
                    json.dumps({"slide": s_idx, "table_index": t_idx, "data": data}, ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )
...
def extract_text_snippets(prs: Presentation, out_dir: Path) -> list[dict]:
    """Extract text content per slide to a Markdown file."""
    out_dir.mkdir(parents=True, exist_ok=True)
    md_path = out_dir / "text-snippets.md"
    snippets = []
    with md_path.open("w", encoding="utf-8") as f:
        ...
        for s_idx, slide in enumerate(prs.slides, 1):
            ...
            snippets.append({"slide": s_idx, "layout": layout, "texts": unique_texts})
            f.write(f"## Slide {s_idx} ({layout})\n\n")
            if unique_texts:
                for t in unique_texts:
                    # escape markdown list chars
                    safe = t.replace("\n", "  \n")
                    f.write(f"- {safe}\n")
            else:
                f.write("_（无文本内容）_\n")
...
def build_index(
    ...
    for s_idx, render_path in enumerate(render_paths, 1):
        slide_images = [img for img in images if img["slide"] == s_idx]
        slide_tables = [t for t in tables if t["slide"] == s_idx]
        slide_texts = next((s["texts"] for s in snippets if s["slide"] == s_idx), [])
        layout = next((s["layout"] for s in snippets if s["slide"] == s_idx), "")
        ...
        slides.append({
            ...
            "images": [f"images/{img['file']}" for img in slide_images],
            "tables": [{"csv": f"tables/{t['csv']}", "json": f"tables/{t['json']}"} for t in slide_tables],
    ...
    with readme.open("w", encoding="utf-8") as f:
        ...
        f.write("\n")
        f.write("## 目录说明\n\n")
        f.write("- `renders/`：每页 PPT 的 PNG 预览（150 DPI），用于快速浏览素材。\n")
        ...
        f.write("|------|------|------|------|\n")
        for s in slides:
            tag_str = ", ".join(s["tags"]) or "render"
            hint = " ".join(s["texts"][:3])[:40]
            f.write(f"| {s['slide']} | {s['layout']} | {tag_str} | {hint} |\n")
...
def main() -> int:
    ...
    if not soffice:
        print("ERROR: soffice not found. Please install LibreOffice or place it under tools/libreoffice/", file=sys.stderr)
        return 1
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        pdf_path = pptx_to_pdf(source_path, tmp_path, soffice)
        print(f"PDF: {pdf_path}")

        print(f"Rendering PDF to PNG ({args.dpi} DPI)...")
        render_paths = pdf_to_pngs(pdf_path, output_dir / "renders", dpi=args.dpi)
    ...
    images = extract_images(prs, output_dir / "images")
    print(f"Extracted {len(images)} images")

    print("Extracting tables...")
    tables = extract_tables(prs, output_dir / "tables")
    print(f"Extracted {len(tables)} tables")

    print("Extracting text snippets...")
    snippets = extract_text_snippets(prs, output_dir)
    print(f"Extracted text from {len(snippets)} slides")

    print("Building index...")
    build_index(output_dir, render_paths, images, tables, snippets, source_path.name)
